...
datafolder = "R:/Projects/Dana/Dana_StarWars_2023_Dups/training_data/must_touch/"
#"G:/Shared drives/Grissom Lab UMN/Projects/Dana/Dana_StarWars_2023_Dups/training_data/must_touch/"
...
for day in dayList:
    ...
    for filename in fileList:
        ...
        for n in Item_Name.index:
            ...
            if Item_Name[n] == "Image Touched": # Pulling "Image Touched" in timestamp from raw csv column DEffectText and adding it to poke times. DANA CHANGED THIS FROM ELIF TO IF AND COMMENTED OUT THE LINE BELOW (now deleted). 
    ...
    df.to_csv(os.path.join(datafolder,day  + "_allAnimals_MustTouch.csv"),index=False)
...
for day in dayList: 
    dayDF = pd.read_csv(os.path.join(datafolder,day  + "_allAnimals_MustTouch.csv"))
    for val in ['choice reaction time', 'retrieval reaction time', 'trial_dur']:
        ...
        dayDF.to_csv(os.path.join(datafolder,day  + "_allAnimals_MustTouch.csv"),index=False)
