...
outputfolder = datafolder + '_choiceRT/'
...
for sessionFolder in os.listdir(datafolder):
    ...
    #I created a for loop to run through the data file searching for the following strings:
        ...
        if len(pokes_time) != len(display_time):
            display_time.pop(len(pokes_time))
        rt_list3 = np.array(pokes_time) - np.array(display_time)
        rt_zscore = stats.zscore(rt_list3)
        #Here I made a matrix and used it to create a dataframe. A data frame was necessary to export as a .csv file.
        bandit_output = {'display timestamp': display_time, 'pokes timestamp': pokes_time, 'choice reaction time': rt_list3, "zscore": rt_zscore}
